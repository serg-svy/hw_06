...
def process_file(file_path, target_directory):
    ...
    if extension in ['jpeg', 'png', 'jpg', 'svg']:
        ...
        unpack_path = os.path.join(target_directory, destination, os.path.splitext(normalized_name)[0])
        shutil.unpack_archive(file_path, unpack_path)

    if destination:
        os.makedirs(os.path.join(target_directory, destination), exist_ok=True)
        new_file_path = os.path.join(target_directory, destination, normalized_name)
